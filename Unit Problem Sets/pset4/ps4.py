...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        numerator = numerator + (y[i] - estimated[i]) ** 2
        denominator = denominator + (y[i] - mean) ** 2
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    # for each model, get the estimated values for x
    for i in range(len(models)):
        estimated = []
        for j in range(len(x)):
            estVal = 0
            for k in range(len(models[i])):
                estVal = estVal + models[i][k] * x[j] ** (len(models[i]) - k - 1)
            estimated.append(estVal)

        # calculate the r_square value
        r_2 = r_squared(y, estimated)
        
        # create a plot for each model
        pylab.figure()
        pylab.clf()
        pylab.plot(x, y, 'bo', label = "data values")
        pylab.plot(x, estimated, 'r', label = "predicted values")
        pylab.title("polynomial degree: {}".format(len(models[i])-1) + "\nr squared: {}".format(r_2))
        pylab.xlabel("year")
